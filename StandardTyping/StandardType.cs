...
namespace RefinementTypes2.StandardTyping
{
    internal class StandardType : IEnumerable<RefinementProduct>
    {
        ...
        public StandardType(IEnumerable<List<StandardRefinement>> bases)
        {
            Products = (from product in bases select new RefinementProduct(product)).ToList();
        }

        public List<List<StandardRefinement>> GetSumOfProductsRefinements() => (from product in Products select product.Refinements).ToList();
        public static StandardType Or(StandardType a, StandardType b) => new StandardType([.. a, .. b]);

        public static StandardType Or(StandardType a, IEnumerable<StandardRefinement> refinements)
        ...
        public static StandardType And(StandardType a, IEnumerable<StandardRefinement> refinements)
